...
from .data.loader import load_data, iter_load_data, list_keys_with_sizes
from .data.preprocess import preprocess_data
from .model.train import BertClassifierTrainer
from .utils.alerts import send_email_alert, format_exception_alert
from .utils.helpers import set_global_seed, setup_logging

def main():
    """Main training pipeline for Immigration Journey Analyzer."""
    # Load configuration
    ...
    logger.info(f" ULTRA-FAST CONFIG: files_per_chunk={files_per_chunk}, rows_per_chunk={rows_per_chunk}")
    ...
    for df_raw, keys in iter_load_data(
        ...
        try:
            ...
            # Read existing results, append new entry, and write back as proper JSON array
                ...
                with open(results_file, "r") as f:
                    try:
                        ...
                        if isinstance(existing_results, list):
                            existing_results.append(chunk_record)
                        else:
                            existing_results = [chunk_record]
            ...
            # Performance monitoring - alert on accuracy drop
            accuracy = metrics.get("accuracy", 0)
            if accuracy < 0.9:  # Alert if accuracy drops below 90%
                logger.error(f"🚨 PERFORMANCE ALERT: Chunk {chunk_index} accuracy {accuracy:.3f} is below 90% threshold!")
                if alert_sender and alert_recipient:
                    subject = f"S3 Classifier Performance Alert - Chunk {chunk_index}"
                    body = f"Performance drop detected:\nChunk: {chunk_index}\nAccuracy: {accuracy:.3f}\nMetrics: {metrics}\nTime: {datetime.utcnow()}"
